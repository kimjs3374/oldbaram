            ...
            def _hook_block_ab(_c):
                ...
                try:
                    ...
                    if _det0 is not None and _gr0 is not None:
                        # 2026-04-25 좌표 변환 정정.
                        # AsyncYolo가 det.x1/y1/x2/y2에 (rx,ry) offset 적용 →
                        # det.cx/cy 는 "mss 프레임 내부 좌표" (rx = gx - ml).
                        # hwnd 캡처일 때 ml/mt = 창 위치(non-zero) 라 det.cx
                        # 자체로는 화면-절대 좌표가 아님.
                        # 화면-절대 = game_region.left + (det.cx - rx).
                        _co0 = getattr(_worker_self,
                                       "_last_crop_offset", (0, 0)) or (0, 0)
                        _rx0 = int(_co0[0])
                        _ry0 = int(_co0[1])
                        _local_cx = int(float(_det0.cx)) - _rx0
                        _local_cy = int(float(_det0.cy)) - _ry0
                        _ax0 = int(_gr0[0]) + _local_cx
                        _ay0 = int(_gr0[1]) + _local_cy
            ...
            while not self._stop:
                ...
                if gr:
                    try:
                        ...
                        if rw2 > 20 and rh2 > 20:
                            ...
                            # 2026-04-25 .copy() 제거: 매 프레임 ~2MB 복사가 fps 급감 원인.
                            # mss는 grab마다 새 버퍼 반환하므로 reference만 들고 있어도 안전.
                            self._last_crop_frame = crop_frame
                ...
                if _locked_now and self._seq_rclick_target is not None \
                    ...
                    try:
                        ...
                        # 2026-04-25 fps 보호: mouse_click_at 내부 sleep 65ms가
                        # 메인 루프 블로킹 → 자힐 동안 fps 급락. 백그라운드 데몬
                        # 스레드로 dispatch (메인 즉시 리턴).
                        import threading as _th
                        _th.Thread(
                            target=mouse_click_at,
                            args=(_tx, _ty),
                            kwargs={"button": "right"},
                            daemon=True,
                        ).start()
                ...
                if (now_sec < self._pending_tab_lock_until
                    ...
                    if now_sec - self._last_map_change_ts >= 0.5 \
                        ...
                        # 2026-04-25 fps 보호: 전체 시퀀스(TAB×2 200ms + relock)
                        # 백그라운드 데몬 스레드로 dispatch. 메인 루프 즉시 리턴.
                        _hmap_snap = self.healer_map
                        _hcoord_snap = self.healer_coord
                        _worker_self = self
                        def _do_tab_lock():
                            try:
                                from ..input.target_sequence import (
                                ...
                                if _worker_self._cycler is not None:
                                    try:
                                        _slots = list(_worker_self._cycler.slots)
                                    except Exception:
                                        pass
                                for _vk in _slots:
                                    try:
                                ...
                                if _worker_self._cycler is not None:
                                    try:
                                        _worker_self._cycler._locked.clear()
                                        _worker_self._cycler._locked.update(_slots)
                                    except Exception:
                                        pass
                                if _worker_self._cycler is not None:
                                    try:
                                        _worker_self._cycler.resume()
                                ...
                                _worker_self.log.info(
                                    ...
                                    f"resume 완료 h_map={_hmap_snap!r} "
                                    f"h={_hcoord_snap} a=({_ax},{_ay}) "
                                    f"dist={_dist}"
                                )
                            except Exception as _te:
                                _worker_self.log.warning(f"[TAB-LOCK] 실패: {_te}")
                        import threading as _th_lock
                        _th_lock.Thread(
                            target=_do_tab_lock, daemon=True
                        ).start()
                ...
                # 2026-04-25 ROLLBACK: 5ms→1ms 변경이 GIL 양보 부족으로 background
                # YOLO predict 60.9→67.3ms 악화시킴. 5ms 원복 — 의도적 GIL release
                # 시간이었음 (background OCR/YOLO 스레드 GPU 점유 보장).
                time.sleep(0.005)
        finally:
