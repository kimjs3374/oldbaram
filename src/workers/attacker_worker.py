                ...
                def _on_cd(rep, src_addr=None):
                    ...
                    try:
                        ...
                        try:
                            ...
                            # 2026-06-15 사용자: 지폭 쿨 20초 이하면 곧 재시전 →
                            # 파력 스킵. 단 0(준비/대기)은 제외 — 첫 시전 전 영구
                            # 0이라 파력 영구 스킵되는 버그 + 준비상태는 쩔캐가
                            # 쏠지 미정. 쿨 도는 중(1~20=최근 쐈고 곧 또)만 스킵.
                            _cj = int(getattr(rep, "cd_jipok", -1))
                            self._jipok_ready_by_idx[_ri] = (1 <= _cj <= 20)
