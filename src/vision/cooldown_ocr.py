...
class CooldownOcr:
    ...
    def _one_cycle(self) -> None:
        ...
        if frame is None:
            ...
            return
        # 2026-04-25 fps 회복: .copy() 제거.
        # AsyncGrabber가 매 iter 새 ndarray 할당, 메인은 reference만 저장.
        # worker는 lock 안에서 frame ref를 자기 변수에 받으므로 메인이
        # submit_frame으로 _latest_frame을 새 ref로 교체해도 worker가
        # 잡은 ref는 별개 ndarray라 race 없음. copy redundant + 큰 비용.
        # 3 인스턴스(cooldown/buff/chat) × frame copy → 메모리 대역폭 점유로
        # 메인 fps 침식. yolo predict 43→60ms 부풀린 주범 후보.
        now = time.time()
        skills: Dict[str, int] = {}
        raw = ""
