...
# 파력무참 시전 판정: 맵명 끝 '(z)' = 층(끝괄호) 기준. 사용자 단언(6/15·6/19).
# ⚠️⚠️ 굴(y)로 절대 바꾸지 말 것 — 6/15·6/17 두 번 바꿨다 욕먹고 복원. z=정답.
# 예 '선비족2-3(5)' → 5(층). jipok/parlyuk 둘 다 층(z) 번호.
_PARLYUK_SUB_RE = re.compile(r"\((\d+)\)\s*$")


def _parlyuk_map_ok(ctx: dict, maps_getter) -> bool:
    """파력무참 시전 허용 맵인지 (층 z=끝괄호 기준, 사용자 단언 6/19).

    maps_getter() 가 빈 집합/None → 전체 굴 허용(기존 동작).
    설정돼 있으면 현재 맵명 끝 '(N)' 의 N(층/끝괄호)이 그 집합에 있을 때만 허용.
