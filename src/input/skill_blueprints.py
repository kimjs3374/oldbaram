...
# 파력무참 시전 판정 (2026-06-17 사용자 C): 굴번호 = 맵명 하이픈 뒤 y 기준
# ('제2선비족5-3(7)' → 굴 3). 끝괄호 (z)=층(lap) 은 무관.
# 기존엔 끝괄호(z) 매칭이라 설정 [3] 이 '(3)' 층 lap 만 잡아, 격수가 같은 굴의
# 다른 lap((1),(7) 등)에서 붙어 기다려도 스킵(돟사36 cast 2회 모두 끝괄호(3))
# 하던 문제 → 굴번호(y) 매칭으로 변경. jipok 은 별도(_jipok_map_ok, z 유지).
_PARLYUK_SUB_RE = re.compile(r"선비족\d+-(\d+)\(")


def _parlyuk_map_ok(ctx: dict, maps_getter) -> bool:
    """파력무참 시전 허용 맵인지 (굴번호 기준, 2026-06-17 사용자 C).

    maps_getter() 가 빈 집합/None → 전체 굴 허용(기존 동작).
    설정돼 있으면 현재 맵 굴번호(하이픈 뒤 y)가 그 집합에 있을 때만 허용.
    예: 설정 {3} → '제2선비족5-3(7)'(굴=3) 허용, '제2선비족5-4(3)'(굴=4) 차단.
    끝괄호 (z)=층 은 무시 → 그 굴의 모든 lap 에서 시전.
    """
    try:
